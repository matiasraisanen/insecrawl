...
from utils.parser_setup import parser_setup
...
class Insecrawl:
    def __init__(self, logLevel=logging.INFO):
        ...
        if args.country:
            self.country = args.country.upper()
        if args.listCountries:
            self.printAmount = True
        if args.details:
            self.cameraDetails["id"] = args.details
            self.printDetails = True
        if args.oneCamera:
            self.cameraDetails["id"]
            self.oneCamera = args.oneCamera
        if args.folder:
            self.downloadFolder = f"images/{args.folder}"
        if args.url:
            self.customURL = args.url
        if args.identifier:
            self.customIdentifier = args.identifier
        if args.scrapeAllCameras:
            self.scrapeAllCams = True
        if args.sortByCountry:
            self.sortByCountry = True
        if args.sortByCamera:
        ...
        if args.version:
            print(f"INSECRAWL v.{self.version}")
            sys.exit(0)
    ...
    def PrintCameraCount(self):
        ...
        self.logger.info("Retrieving list of online cameras...")
        print("╔══════╦══════╦═══════════════════════════╗")
        ...
        for key in sorted(newList.keys()):
            ...
            print("║  {}  ║ {} ║ {}║".format(countryCode, cameraQuantity, countryName))
        try:
            count = str((newList["Unknown location"]["count"])).rjust(4, " ")
            print(
                "║   {}  ║ {} ║ {}║".format(
                    newList["Unknown location"]["code"],
                    count,
                    newList["Unknown location"]["country"].ljust(26, " "),
                )
            )
        except:
            pass
        print("╚══════╩══════╩═══════════════════════════╝")
        self.logger.info(
            "Found a total of {} cameras from {} countries on insecam.org".format(
    ...
    def RaiseCritical(self):
        ...
        self.logger.critical("Program encountered a critical error and must quit.")
    ...
    def GetMaxPageNum(self):
        ...
        try:
            url = "http://www.insecam.org/en/bycountry/{}/".format(self.country)
            headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.3"
            }
            req = Request(url=url, headers=headers)
            ...
            self.logger.error(
                "Country code {} ({}) returned 404! Insecam has no cameras from this country".format(
    ...
    def GetDetails(self):
        ...
        url = "http://www.insecam.org/en/view/{}/".format(self.cameraDetails["id"])
        ...
        try:
            ...
            for script in soup.find_all("script"):
                match = re.findall(r"addtagset\(\"(\w+)\"\);", script.get_text())
                if match:
    ...
    def PrintDetails(self):
        ...
        print("╠══════════════╣")
        print("║ Insecam URL  ╟╴ {}".format(self.cameraDetails["insecamURL"]))
        print("╠══════════════╣")
        print("║  Direct URL  ╟╴ {}".format(self.cameraDetails["directURL"]))
        print("╚══════════════╝")
    ...
    def WriteImage(self, cameraID, cameraURL, downloadFolder, totalCams):
        ...
        if success:
            ...
            if self.sortByCamera:
                ...
                cv2.imwrite(
                    f"{downloadFolder}/{cameraID}/[{cameraID}]_{timestampStr}.jpg",
                    image,
                )
                self.logger.debug(
    ...
    def DownloadCustomURL(self):
        ...
        try:
            ...
            self.WriteImage(
                self.customIdentifier, self.customURL, self.downloadFolder, 1
            )
    ...
    def ScrapeOne(self, cameraID):
        ...
        url = "http://www.insecam.org/en/view/{}/".format(cameraID)
        self.cameraDetails["insecamURL"] = url
        ...
        try:
            ...
            for img in soup.findAll("img"):
                if img.get("id") == "image0":
                    self.logger.debug("START processing camera ID {}".format(cameraID))
                    image_url = img.get("src")
                    self.logger.debug("Image URL: {}".format(image_url))
                    self.WriteImage(cameraName, image_url, self.downloadFolder, 1)
    ...
    def ScrapeAllCameras(self):
        ...
        for key in sorted(self.countriesJSON.keys()):
            ...
            totalCountries += 1
        self.logger.info(
            "Found {} cameras from {} countries. This could take a long time.".format(
        ...
        try:
            ...
            for img in images:
                ...
                if "yandex" in image_url:
                    ...
                    self.logger.debug("DONE processing img ID{}".format(image_id))
                    continue
                if img.get("id") == None:
                ...
                if self.newCamerasOnly:
                    if not self.ImageExists(image_id):
                        p = multiprocessing.Process(
                            target=self.WriteImage,
                            args=(image_id, image_url, self.downloadFolder, totalCams),
                        )
                    else:
                        self.LoadingBar(self.progressCounter.value, totalCams)
    ...
    def ScrapePages(self, countryCode, countryName):
        ...
        self.logger.info("Scraping images from cameras in {}.".format(countryName))
        ...
        while page <= int(self.maxPages):
            ...
            page += 1
        self.logger.info("Done scraping cameras in {}.".format(countryName))
        self.logger.info("Images downloaded: {}".format(self.successfulScrapes.value))
        # self.successfulScrapes = 0

        if self.skippedImages.value > 0:
            self.logger.info("Skipped cameras: {}".format(self.skippedImages.value))
            self.skippedImages.reset()
        if self.erroredScrapes.value > 0:
            self.logger.info("Failed scrapes: {}".format(self.erroredScrapes.value))
            self.erroredScrapes.reset()
        self.logger.info("Images saved in {}".format(self.downloadFolder))
    ...
    def main(self):
        ...
        if self.country:
            self.logger.debug(
                "Country code {} resolved to {}.".format(self.country, self.countryName)
            )
            self.ScrapePages(self.country, self.countryName)
            ...
            if self.interval != 0:
                while True:
                    ...
                    for i in range(self.interval):
                        timeleft = str((self.interval - i)).zfill(
                            len(str(self.interval))
                        )
